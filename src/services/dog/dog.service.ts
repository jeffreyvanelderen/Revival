...
export class DogService {
  ...
  getBreeds(params: { limit?: number; page?: number } = { limit: 10, page: 0 }): Observable<ExternalBreedsResponse> {
    return this.http.get<ExternalBreedsResponse>(`${this.baseURL}/${this.breedsEndpoint}`, {
  ...
  getBreed(id: number): Observable<ExternalBreed> {
    return this.http.get<ExternalBreed>(`${this.baseURL}/${this.breedsEndpoint}/${id}`, {
      headers: this.getHeaders(),
    })
  }
